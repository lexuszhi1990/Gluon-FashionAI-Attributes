...
class MaxPoolPad(nn.HybridBlock):
    ...
    def __init__(self, kernel_size=3, strides=2, padding=1):
        super(MaxPoolPad, self).__init__()
        self.pool = nn.MaxPool2D(pool_size=(kernel_size,kernel_size), strides=strides, padding=padding)
...
class AvgPoolPad(nn.HybridBlock):
    ...
    def __init__(self, kernel_size=3, strides=2, padding=1):
        ...
        self.pool = nn.AvgPool2D(pool_size=(kernel_size, kernel_size), strides=strides, padding=padding)
...
class NASNetALarge(nn.HybridBlock):
    ...
    def __init__(self, num_classes=1001):
        ...
        self.conv0.add(nn.Conv2D(channels=96, kernel_size=3, padding=0, strides=2, use_bias=False))
...
def nasnet_large(**kwargs):
    model = NASNetALarge(100)
    return model

if __name__ == '__main__':
    net = nasnet_large()
    net.initialize()
    data = mx.nd.ones((2, 3, 331, 331))
    y = net(data)
    print(y.shape)
